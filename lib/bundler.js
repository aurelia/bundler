...
import {readConfig, serializeConfig} from './config-serializer';

export function bundle(cfg) {

  let appCfg = readConfig(fs.readFileSync(cfg.configPath, 'utf8'));
  ...
  return builder.trace(moduleExpression)
    ...
    .then((output) => {
      ...
      let outfile = getOutFileName(output, cfg.bundleName + '.js', cfg.options.rev);
...
function injectBundle(builder, output, outfile, appCfg, cfg) {
  ...
  fs.writeFileSync(cfg.configPath, serializeConfig(appCfg));
}
