...
export function bundle(modules, excludes, fileName, _opts) {
  ...
  let moduleExpression = modules.map(m => getFullModuleName(m, config.loader.__originalConfig.map)).join(' + ');
  ...
  return Promise
    .all([builder.trace(moduleExpression), builder.trace(excludeExpression)])
    .then((trees) => {
      let tree = builder.subtractTrees(trees[0], trees[1]);
